...
| 機能 | 実装 |
|------|------|
| root専用ファイルの編集/操作 | 通常権限で書き込み/作成/削除/リネーム/移動が失敗した場合、su が使える端末では自動的に `su` でフォールバックして実行。root でしか触れない `/data/data`・`/system` 内のファイルも編集・保存できる (ルートモードを明示 ON にしていなくても su があればフォールバック)。保存時は「保存しました (root) 🔓」と表示 |

### v1.3.2 で修正

- 「権限を取得してテスト」ボタンを常に表示し、実際に `su -c id` を実行して判定するよう変更 (Magisk 等で su が固定パスに無くても検証できる)。
