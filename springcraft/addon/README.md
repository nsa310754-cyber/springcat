...
### それでも直らない場合に確認してほしいこと

- **新しいワールドで試す**: 既存ワールドに一度アドオンを適用していた場合、
  ワールド側が古いパックの参照を保持したままになっていることがあります。
  新規ワールドを作成し、ワールド設定の「アドオン」で Behavior Pack /
  Resource Pack を両方追加・有効化してみてください。
- **インポート確認画面が本当に出ているか**: アプリの「Minecraft にアドオンを追加」を
  タップした際に、Minecraft 側で `.mcaddon` の取り込み確認ダイアログが表示されて
  いるかどうか。表示されていない場合は JSON の中身以前に、Android 側の Intent
  受け渡し(MIME タイプ等)の問題である可能性があります。
- **Minecraft のバージョン**: 設定画面などで確認できる実際のバージョン番号
  (`26.xx` のような表記、または `1.21.xx`)を教えてもらえると、
  `min_engine_version` をその環境に合わせてさらに調整できます。
