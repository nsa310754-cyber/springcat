...
### ✨ 追加機能
- **PCサイト表示（User-Agent Switcher）**: 既定でデスクトップ UA。PC限定表示のサイトも
  PC版として開けます。右上 **⋮メニュー → 「PCサイト表示（デスクトップUA）」** で
  モバイル表示と切り替え（チェックのON/OFF、切替後は自動リロード）。
- **ダウンロード表示**: サイト内のダウンロードボタンを押すと、画面下部に
  **「〇〇 をダウンロードしています…」** とファイル名付きで表示され、実ファイルは
  端末の「ダウンロード」フォルダへ保存されます。
