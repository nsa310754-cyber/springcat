...
古い APK を **最新の Android 端末にインストールできる形に変換する**ツールです。
AI・クラウドは一切使わず、完全に決定論的な処理だけで動きます。

このリポジトリには 2 つの実装があります。

| 場所 | 形態 | 使う場面 |
| --- | --- | --- |
| [`android/`](android/) | **Android アプリ本体(APK)** | スマホ単体で完結させたいとき。古い APK を選ぶ→変換→インストール |
| [`fixapk/`](fixapk/) | **PC 用 CLI / GUI(Python)** | PC でまとめて変換したいとき(JDK の jarsigner を使用) |

どちらも処理内容は同じ:マニフェストの SDK バージョンを引き上げ、旧署名を除去し、署名し直します。
